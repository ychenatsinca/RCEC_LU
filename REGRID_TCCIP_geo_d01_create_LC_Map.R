...
source("fun_regrid_tccip_d01.R")

#load tccip d01 geo data
d01.fname = "../geo_em.d01.nc"
d01.geo <- fun_read_nc(arg1=d01.fname)

mask_arr_a  <- d01.geo$LANDMASK
nav_lon_a   <- d01.geo$CLONG
nav_lat_a   <- d01.geo$CLAT
...
lcfrac <- d01.geo$LANDUSEF
lcdom  <- d01.geo$LU_INDEX
# set resolution at 10km
dx=0.1
dy=0.1
...
#	}
 #       if ( (bn.2km[i,j] > 0.5) & (bn.2km[i,j] <= 0.7) ) {
	      	lcfrac[i,j,13] <- tmp.frac*(bn.10km[i,j])
		lcfrac[i,j,14] <- tmp.frac*(1.-bn.10km[i,j])*0.3
		lcfrac[i,j,15] <- tmp.frac*(1.-bn.10km[i,j])*0.7
#	}
#	if ( bn.2km[i,j] > 0.7) {
...
if (ld.create.nc) {
  ...
  dim3 <- ncdim_def("west_east","",  1:249, create_dimvar=FALSE) 
  dim4 <- ncdim_def("south_north","",1:249, create_dimvar=FALSE)
  dim5 <- ncdim_def("south_north_stag","",1:250, create_dimvar=FALSE)
  dim6 <- ncdim_def("west_east_stag","",1:250,   create_dimvar=FALSE) 
  ...
  dim10<- ncdim_def("num_urb_params","",1:132, create_dimvar=FALSE)
  #============================================================= 
  mv <- 1e+10 
  ...
  var54 <- ncvar_def(name="URB_PARAM",units="dimensionless",dim=list(dim3,dim4,dim10,dim1),missval=mv, prec="float")
  #==== assign file name ===
  out_filename <- paste("d01.geo_RCEC_",work.year,".nc",sep="") 
  ...
  org.data <- fun_read_nc(arg1=d01.fname,arg2=FALSE)
